...
launch_ios_app_on_device() {
    ...
    xcrun devicectl device process launch \
        --device "$device_name" \
        --terminate-existing \
        "$IOS_APP_BUNDLE_ID" >/tmp/goaway-devicectl-launch.log 2>&1
}
...
count_ios_apple_development_identities() {
    security find-identity -v -p codesigning 2>/dev/null \
        | grep -c "Apple Development" || true
}

has_xcode_cli_account_credentials() {
    if security find-generic-password -s Xcode-Username >/dev/null 2>&1; then
        return 0
    fi

    local xcode_accounts
    xcode_accounts=$(defaults read com.apple.dt.Xcode DVTDeveloperAccountManagerAppleIDLists 2>/dev/null || true)
    if echo "$xcode_accounts" | grep -q "IDE.Identifiers.Prod"; then
        if ! echo "$xcode_accounts" | grep -q "IDE.Identifiers.Prod =     ("$'\n'"    );"; then
            return 0
        fi
    fi
...
has_local_provisioning_profiles() {
    if find "$HOME/Library/Developer/Xcode/UserData/Provisioning Profiles" -maxdepth 2 \( -name '*.mobileprovision' -o -name '*.provisionprofile' \) 2>/dev/null | grep -q .; then
        return 0
    fi

    find "$HOME/Library/MobileDevice" -maxdepth 3 \( -name '*.mobileprovision' -o -name '*.provisionprofile' \) 2>/dev/null | grep -q .
}

check_ios_device_signing_ready() {
    local development_identity_count
    development_identity_count=$(count_ios_apple_development_identities)
    development_identity_count="${development_identity_count:-0}"

    if [ "$development_identity_count" = "0" ]; then
        print_error "当前 Mac 上没有可用的 Apple Development 证书，无法直接命令行安装到真机"
        print_info "长期可用的正式做法："
        ...
        print_info "4. Team 选择你的 Apple Developer Program 团队（不要再用 Personal Team）"
        print_info "5. 保持 Bundle Identifier 为唯一值，例如 com.zhaoqiang.whattoeat.dev"
        print_info "6. 在 Xcode -> Settings -> Accounts -> Manage Certificates... 中确保存在 Apple Development 证书"
        print_info "7. 连接 iPhone 后，在 Xcode 里点一次 Run，让开发证书和 provisioning profile 同步到本机"
        exit 1
    fi

    if ! has_xcode_cli_account_credentials; then
        print_warning "未检测到稳定的 Xcode CLI 账号凭据，将继续尝试使用本机已有证书和 provisioning profile 构建真机包"
        print_info "如果后续 xcodebuild 仍报 No Accounts / missing Xcode-Username，再回到 Xcode -> Settings -> Accounts 重新登录账号"
    fi

    if ! has_local_provisioning_profiles; then
        print_error "当前 Mac 上还没有任何本地 provisioning profile，命令行安装真机会失败"
        print_info "正式修复步骤："
        print_info "1. 在 Xcode 中使用目标 Team 和 Bundle Identifier 对真机点一次 Run"
        print_info "2. 确认 Xcode 已自动创建并下载 iOS App Development profile"
        print_info "3. 如仍未生成，可在 Xcode -> Settings -> Accounts 中选择团队后点 Download Manual Profiles"
        exit 1
    fi

    if [ -n "$IOS_DEVELOPMENT_TEAM" ]; then
        print_info "使用 IOS_DEVELOPMENT_TEAM: $IOS_DEVELOPMENT_TEAM"
    else
        print_warning "未设置 IOS_DEVELOPMENT_TEAM，建议在 frontend/.env.local 中显式配置当前 Apple Developer Team ID"
    fi
}
...
start_device() {
    ...
    # 过滤掉 Apple 工具链已知的噪音日志（不影响真正的错误）:
    #   - xcodebuild 在未指定具体 destination id 时打印的 "Using the first of multiple matching destinations" 警告及其候选列表
    #   - CoreDevice 的 "No provider was found" / "devicectl manage create" 提示（安装/拉起时都会打印，Apple 自家工具也有）
    local noise_filter='/^--- xcodebuild: WARNING: Using the first of multiple matching destinations:/d;/^[[:space:]]*{ platform:/d;/Failed to load provisioning paramter list/d;/devicectl manage create/d'
    ...
    npx react-native run-ios --mode Release --no-packager --device "$resolved_device_name" \
        --extra-params "$ios_extra_params" 2>&1 | tee "$device_run_log" | sed -E "$noise_filter"
    ...
    if grep -q "success Installed the app on the device." "$device_run_log"; then
        if launch_ios_app_on_device "$resolved_device_name"; then
            ...
            print_info "devicectl 日志: /tmp/goaway-devicectl-launch.log"
        fi
    fi
}
...
start_android() {
    ...
    APP_PLATFORM=android write_runtime_config
    start_metro_background

    # 确保 adb 在 PATH 中（react-native CLI 启动 App 时也需要）
    ...
    if ! adb devices | grep -q "device$"; then
        print_warning "未找到Android设备或模拟器，请先启动模拟器或连接真机"
    fi

    # general flavor 用于日常开发调试，release 包按渠道单独构建
