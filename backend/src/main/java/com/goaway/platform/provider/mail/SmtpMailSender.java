...
public class SmtpMailSender implements MailSender {
    private static final String DEFAULT_BRAND_NAME = "What To Eat";
    private static final String DEFAULT_RESET_SUBJECT = "今天吃点啥 密码找回验证码";
    ...
    public SmtpMailSender(
            ...
            @Value("${app.mail.from-name:What To Eat}") String fromName,
    ...
    public void sendEmailChangeCode(String email, String code) {
        ...
        try {
            ...
            helper.setTo(email);
            helper.setSubject("今天吃点啥 邮箱更换验证码");
            helper.setText(buildEmailChangeBody(code), false);
            javaMailSender.send(message);
        } catch (MailException | MessagingException | UnsupportedEncodingException exception) {
            throw new EmailDeliveryException(
                    EmailDeliveryException.FailureType.DELIVERY,
    ...
    private String buildEmailChangeBody(String code) {
        String brandName = fromName.isBlank() ? DEFAULT_BRAND_NAME : fromName;
        return """
                %s 邮箱更换验证码
                ...
                如果这是你本人发起的邮箱更换操作，请返回应用并输入以上验证码继续操作。
                如果不是你本人发起，请直接忽略本邮件；你的账号信息不会因此被修改。
