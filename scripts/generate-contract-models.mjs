...
function isClientContractVisible(field) {
  return !field.annotations.some((annotation) => annotation.startsWith('@JsonIgnore'));
}

function parseJavaClass(filePath) {
  const content = fs.readFileSync(filePath, 'utf8');
  const classMatch = content.match(/public\s+(?:final\s+)?(?:class|record)\s+([A-Za-z0-9_]+)/);
  if (!classMatch) {
    throw new Error(`Could not determine class name for ${filePath}`);
  }

  const isRecord = /public\s+(?:final\s+)?record\s+/.test(content);
  const fields = [];

  if (isRecord) {
    // Record components live in the header's parameter list, not the body.
    // Scan for the opening paren after the class name, then grab balanced content.
    const nameIdx = content.indexOf(classMatch[1]);
    const parenStart = content.indexOf('(', nameIdx);
    if (parenStart !== -1) {
      let depth = 1;
      let i = parenStart + 1;
      ...
      const paramBlock = content.slice(parenStart + 1, i - 1);
      for (const param of paramBlock.split(',')) {
        // Strip annotations like @Nullable, @JsonInclude(...) etc.
        const stripped = param.trim().replace(/@[A-Za-z0-9_]+(?:\([^)]*\))?\s*/g, '').trim();
        const paramMatch = stripped.match(/^(.+?)\s+([A-Za-z0-9_]+)$/);
        if (paramMatch) {
          fields.push({
            name: paramMatch[2],
            javaType: paramMatch[1].replace(/\s+/g, ' ').trim(),
            annotations: [],
          });
        }
      }
...
function main() {
  ...
  const dtoModels = dtoFiles.map((filePath) => {
    ...
    return {
      ...parsed,
      fields: parsed.fields.filter(isClientContractVisible),
  ...
  const entityReport = config.entityContracts.map((mapping) => {
    ...
    const expectedSharedFields = entity.fields.filter((field) => {
      ...
      return isClientContractVisible(field);
    });

    const visibleDtoFields = dto.fields.filter(isClientContractVisible);
    const dtoFieldByName = new Map(visibleDtoFields.map((field) => [field.name, field]));
    const problems = [];
    ...
    for (const field of visibleDtoFields) {
