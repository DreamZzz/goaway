...
| 菜谱图片 | `APP_LLM_IMAGE_PROVIDER` | `disabled` | `jimeng`,`openai-compatible`,`web-search` | 生产用即梦 AI 生成，主链路支持异步补图，失败不阻塞主请求 |
| 食材库存识别 | `APP_INVENTORY_VISION_PROVIDER` | `mock` | `bailian-qwen-vl` | Pro 拍照识别食材库存，生产需配置百炼 Qwen-VL API key |
